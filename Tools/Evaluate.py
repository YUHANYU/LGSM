...
def MLL_Evaluate(y_pred, y_real, val_test):

    hm = round(hamming_loss(y_true=y_real, y_pred=y_pred), ndigits=3) 
    # f1_score_micro = round(f1_score(y_true=y_real, y_pred=y_pred, average='micro'), ndigits=3) 
    # f1_score_macro = round(f1_score(y_true=y_real, y_pred=y_pred, average='macro'), ndigits=5) 
    # auc = round(roc_auc_score(y_true=y_real, y_score=y_pred), ndigits=5) #roc
    # f1_score_macro_2 = round(MacroAveragingAUC(outputs=y_pred, test_target=y_real), ndigits=5) 
    ...
    if val_test == 'test': 
        ...
        return (hm, one_error, coverage, rank_loss, average_precision)
    else: 
        return (hm, one_error, coverage, rank_loss, average_precision)


def MLL_Mean_Std(results):
    ...
    print('0.HammingLoss↓', '1.One-error↓', '2.Coverage↓', '3.RankingLoss↓', '4.Average Precision↑')
    for i in range(len(mean_std)):
        print(i, 'mean+std: %4.3f' % mean_std[i][0], '+', '%4.3f' % mean_std[i][1])
